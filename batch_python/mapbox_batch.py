...
class MapboxBatchGeocoder(object):
    ...
    def _send(self, chunk, path):
        response = requests.get('https://api.mapbox.com/geocoding/v5/mapbox.places-permanent/{}.json?access_token={}'.format(';'.join([quote_plus(s.strip()) for s in chunk]), self.mapbox_access_token))
